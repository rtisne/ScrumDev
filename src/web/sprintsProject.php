...
function createSprint() {
    ...
    if (!empty($title) ) {
        ...
        $number_Sprint = get_number_sprint(intval($_GET[GET_ID_PROJECT]));
        $number_Sprint_Actual = intval($number_Sprint["num"]) + 1;
        $safe_values = array("number" => $number_Sprint_Actual, "title" => $title , "date_start"=>$date_start, "date_end"=>$date_end, "id_project"=>intval($_GET[GET_ID_PROJECT]));
		$sprint_id = add_sprint_in_db($safe_values);

        //Create the ALL userstory container for the sprint
        $safe_values = array("number" => -1, "title" => "ALL" , "description" => "", "is_all"  => 1, "state" => 0, "id_project" => intval($_GET[GET_ID_PROJECT]));
